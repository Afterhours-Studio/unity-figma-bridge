...
namespace UnityFigmaBridge.Editor.Fonts
{
    ...
    public static class GoogleFontLibraryManager
    {
        ...
        public static void LoadFontData()
        {
            ...
            // Find google-fonts.json wherever it lives in the project (local package, Assets, etc.)
            string jsonText = null;
            foreach (var guid in AssetDatabase.FindAssets("google-fonts"))
            {
                var assetPath = AssetDatabase.GUIDToAssetPath(guid);
                if (!assetPath.EndsWith("google-fonts.json")) continue;
                var ta = AssetDatabase.LoadAssetAtPath<TextAsset>(assetPath);
                if (ta != null) { jsonText = ta.text; break; }
                // Fallback: read via filesystem (e.g. local package not imported as TextAsset)
                var fullPath = Path.GetFullPath(assetPath);
                if (File.Exists(fullPath)) { jsonText = File.ReadAllText(fullPath); break; }
            }
            if (jsonText == null)
            {
                Debug.LogError("[FigmaBridge] google-fonts.json not found anywhere in the project");
                s_FontDefinitions = new Dictionary<string, GoogleFontDefinition>();
                return;
            }
            s_FontDefinitions = JsonConvert.DeserializeObject<Dictionary<string, GoogleFontDefinition>>(jsonText);
            Debug.Log($"[FigmaBridge] Google Fonts loaded: {s_FontDefinitions.Count} families");
        }
        ...
        private static GoogleFontDefinition GetFontDefinition(string fontName)
        {
            ...
            var exact = s_FontDefinitions.FirstOrDefault(kv =>
