...
the specified screen as per the Figma document.
...
| **Vectors**             | Rendered on the server as a PNG (see *Server Rendering* below)                                                         |

## Fonts

With the goal of trying to make it 1-click sync, if the font doesnt exist in your project it will try and download a
matching TTF font from Google Fonts and generate a new TextMesh Pro Font to match.
...
As a result of the way that TextMeshPro handles outlines, the package will apply a custom shader (Figma/TextMeshPro)
...
* All used textures must have "sRGB" checked. The importer will automatically do this but if you are
