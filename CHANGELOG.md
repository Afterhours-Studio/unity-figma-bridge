...
### Added
- **Gradient fills** — linear and radial gradient fills now render natively via `FigmaImage` and the SDF shader (up to 16 stops). Previously these fell back to solid color.
- **Text TRUNCATE mode** — Figma `TEXT_TRUNCATE` auto-resize maps to TMP `TextOverflowModes.Ellipsis` + `TextWrappingModes.Normal`.
- **Force Sync button** — bypasses incremental hash check and re-downloads all assets for selected frames regardless of whether content changed.
- **Character Spacing mode** — new `CharacterSpacingMode` setting (Fixed / Auto). Fixed applies a constant offset (default −0.7, same as before). Auto reads `letterSpacing` directly from the Figma node.
- **Version badge** — editor window header now shows the real package version read from `package.json` at runtime instead of hardcoded `v1.0`.
- **Cached GUIStyles** — all editor window styles rebuilt once per skin change, eliminating ~37 per-frame allocations.
- **Partial class split** — `FigmaBridgeEditorWindow` split into focused partial files (`Import`, `Build`, `Settings`, `Log`) inside `Editor/EditorWindow/`.

### Fixed
- Image fill component was added unconditionally before checking whether a sprite loaded successfully, resulting in a dangling `Image` with no sprite on nodes where the asset was missing.
- Tab bar animated accent line snapped instead of sliding when switching to the Import tab (index 0) — replaced `-1f` sentinel with a `bool _tabLineInitialized` flag and capture `_tabLineFromX = _tabLineX` at click time.
- `FigmaImage.CornerRadius` serialized as separate `[Range(0,1)]` floats (`m_CornerTL/TR/BR/BL`) so individual corners are editable in the Inspector.
- Static `GUIStyle` fields (`s_FrameName`, `s_RowBtnLabel`, `s_SectionName`, `s_FrameBadge`) were mutated per-row, risking color leak between tabs — each site now uses a local copy or restores the default after use.
- Mono font for token field was loaded via `EditorGUIUtility.Load` every frame — now cached in `EnsureStyles()`.
- Dead fields `_buildCacheLoaded` / `_buildCacheInfo` removed.
