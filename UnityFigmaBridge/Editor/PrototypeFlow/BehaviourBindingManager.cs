...
namespace UnityFigmaBridge.Editor.PrototypeFlow
{
    public static class BehaviourBindingManager
    {
        ...
        /// <summary>
        /// Cache for type lookups to avoid scanning all assemblies repeatedly
        /// </summary>
        ...
        {
            if (caseInsensitive && String.Equals(transform.name, nameMatch, StringComparison.OrdinalIgnoreCase)) return true;
        ...
        {
            var cacheKey = $"{nameSpace}:{name}".ToLowerInvariant();
                ...
                foreach (Type type in assembly.GetTypes())
                {
                    if (String.Equals(type.Name, name, StringComparison.OrdinalIgnoreCase))
                    {
                        if (nameSpace.Length > 0)
                        {
                            ...
                            if (!String.Equals(nameSpace, type.Namespace, StringComparison.OrdinalIgnoreCase))
                            {
                                s_TypeCache[cacheKey] = null;
                                return null;
                            }
                        }
                        s_TypeCache[cacheKey] = type;
                        return type;
                    }
                }
            }
